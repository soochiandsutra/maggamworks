...
export default function FrontOthersSection() {
  const { frontSelectedTechniques, setFrontSelectedTechniques } = useAppStateStore();
  ...
  const handleTechniqueChange = (technique: string, checked: boolean) => {
    if (checked) {
      setFrontSelectedTechniques(prev => [...prev, technique]);
    } else {
      setFrontSelectedTechniques(prev => prev.filter(t => t !== technique));
    }
  };
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {embroideryTechniques.map((technique) => (
              <div key={technique} className="flex items-center space-x-2">
                <Checkbox
                  id={technique.toLowerCase().replace(/\s+/g, '-').replace(/\//g, '-') + '-front'}
                  checked={frontSelectedTechniques.includes(technique)}
                  onCheckedChange={(checked) => handleTechniqueChange(technique, checked as boolean)}
                />
                <Label
                  htmlFor={technique.toLowerCase().replace(/\s+/g, '-').replace(/\//g, '-') + '-front'}
                  className="text-sm"
                >
                  {technique}
                </Label>
