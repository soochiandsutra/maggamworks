...
export default function FrontNeckSection() {
  const {
    ...
    setFrontNeckType,
    setFrontNeckDesignNumber,
    setFrontNeckType2,
    setFrontNeckType2DesignNumber,
  } = useAppStateStore();

  return (
    <div className="grid gap-4">
      <h3 className="font-semibold mb-3 flex items-center gap-2 text-base">
        <span className="text-primary">👔</span> Front Neck Details
      </h3>
      <div className="space-y-6">
        <div className="space-y-3">
          <Label className="text-sm font-medium">Neck Type</Label>
          <RadioGroup value={frontNeckType} onValueChange={setFrontNeckType} className="grid grid-cols-1 gap-3 md:grid-cols-3">
            {neckTypes.map((neck) => (
              <IconCard
                key={neck.id}
                item={neck}
                isSelected={frontNeckType === neck.id}
                onClick={() => setFrontNeckType(neck.id)}
        ...
        <div className="space-y-3">
          ...
          <RadioGroup value={frontNeckDesignNumber} onValueChange={setFrontNeckDesignNumber} className="grid grid-cols-3 gap-3 sm:grid-cols-5">
            {[1, 2, 3, 4, 5].map((num) => (
              <Card
                key={num}
                className={`relative cursor-pointer p-3 gap-2 transition-all ring-1 flex items-center justify-center ${
                  frontNeckDesignNumber === num.toString()
                    ? "border-primary ring-primary bg-primary/5"
                    : "border-border ring-1 ring-border/30 hover:border-primary/30 hover:bg-primary/5"
                }`}
                onClick={() => setFrontNeckDesignNumber(num.toString())}
              >
                ...
                <h3
                  className={`text-sm font-medium ${
                    frontNeckDesignNumber === num.toString() ? "text-primary" : "text-muted-foreground"
                  }`}
                >
                  {num}
                </h3>
        ...
        <div className="space-y-3">
          ...
          <RadioGroup value={frontNeckType2} onValueChange={setFrontNeckType2} className="grid grid-cols-2 gap-3">
            {neckType2Data.map((neck) => (
              <ImageCard
                key={neck.id}
                item={neck}
                isSelected={frontNeckType2 === neck.id}
                onClick={() => setFrontNeckType2(neck.id)}
                radioId={`front-neck2-${neck.id}`}
              />
            ))}
          </RadioGroup>
        </div>
        ...
        <div className="space-y-3">
          ...
          <RadioGroup value={frontNeckType2DesignNumber} onValueChange={setFrontNeckType2DesignNumber} className="grid grid-cols-2 gap-3">
            {[1, 2, 3, 4, 5, 6].map((num) => (
              <ImageCard
                key={num}
                item={{ id: num, name: `Design ${num}` }}
                isSelected={frontNeckType2DesignNumber === num.toString()}
                onClick={() => setFrontNeckType2DesignNumber(num.toString())}
                radioId={`front-neck2-design-${num}`}
                altPrefix="Neck "
              />
            ))}
          </RadioGroup>
