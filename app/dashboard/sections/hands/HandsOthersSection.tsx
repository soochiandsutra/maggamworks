...
export default function HandsOthersSection() {
  ...
  const handleTechniqueChange = (technique: string, checked: boolean) => {
    if (checked) {
      setHandsSelectedTechniques([...handsSelectedTechniques, technique]);
      // Initialize percentage to 50% if not already set
      if (!handsTechniquePercentages[technique]) {
        setHandsTechniquePercentage(technique, 50);
      }
    } else {
      setHandsSelectedTechniques(handsSelectedTechniques.filter(t => t !== technique));
    }
  };
                ...
                <Card key={technique} className="p-4 ring-1 border-border ring-border/30">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-sm text-muted-foreground">
                      {handsTechniquePercentages[technique] || 50}%
                    </span>
                  </div>
                  <Slider
                    value={[handsTechniquePercentages[technique] || 50]}
                    onValueChange={(value) => setHandsTechniquePercentage(technique, value[0])}
                    max={100}
                    min={0}
                    step={1}
                    className="w-full"
          ...
          {embroideryTechniques.map((technique) => (
            <Card
              ...
              className={`relative cursor-pointer p-4 ring-1 transition-all ${
                handsSelectedTechniques.includes(technique.name)
                  ? "border-primary ring-primary bg-primary/5"
                  : "border-border ring-border/30 hover:border-primary/30 hover:bg-primary/2"
              }`}
              onClick={() => handleTechniqueChange(technique.name, !handsSelectedTechniques.includes(technique.name))}
            >
              <div className="absolute right-3 top-3">
                <Checkbox
                  id={technique.id + '-hands'}
                  checked={handsSelectedTechniques.includes(technique.name)}
                  onChange={() => {}}
                  className="h-5 w-5"
                />
              </div>
