      ...
      <div className="space-y-4">
        ...
        {handsHasFillWork && (
          <div className="space-y-3">
            ...
            <RadioGroup value={handsCoverage} onValueChange={setHandsCoverage} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-10 gap-3">
              {coverageOptions.map((coverage) => (
                <TextCard
                  key={coverage.id}
                  item={{ ...coverage, name: `${coverage.name} coverage` }}
                  isSelected={handsCoverage === coverage.id}
                  onClick={() => setHandsCoverage(coverage.id)}
                  radioId={`hands-coverage-${coverage.id}`}
                />
              ))}
            </RadioGroup>
          </div>
